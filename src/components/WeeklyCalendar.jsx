...
export default function WeeklyCalendar({ plan, onAddMeal, onRemoveMeal }) {
                  ...
                  <td key={day} className="align-top py-1">
                    <div className="min-h-[64px] bg-gray-50 rounded-lg p-1.5 border border-dashed border-gray-200 space-y-1">
                      {planLookup[day][mt].map(entry => {
                        ...
                        const result = calcMealEmissions(entry.mealId);
                        if (!meal) return null;
                        return (
                          <div
                            key={entry.id}
                          ...
                          >
                            ...
                            {result && (
                              <EmissionBadge kgCO2={result.totalKgCO2} />
                            )}
                          </div>
                        );
                      })}
...
function MealSlot({ day, mealType, entries, onAdd, onRemove }) {
  return (
    <div>
